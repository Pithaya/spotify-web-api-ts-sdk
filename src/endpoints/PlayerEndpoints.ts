...
export default class PlayerEndpoints extends EndpointsBase {
    public async getPlaybackState(market?: Market, additional_types?: string) {
        const params = this.paramsFor({ market, additional_types });
        return await this.getRequest<PlaybackState>(`me/player${params}`);
    }

    public async getAvailableDevices() {
        return await this.getRequest<Devices>("me/player/devices");
    }

    public async getCurrentlyPlayingTrack(market?: Market, additional_types?: string) {
        const params = this.paramsFor({ market, additional_types });
        return await this.getRequest<PlaybackState>(`me/player/currently-playing${params}`);
    }

    public async getRecentlyPlayedTracks(limit?: MaxInt<50>, queryRange?: QueryRange) {
        ...
        return await this.getRequest<RecentlyPlayedTracksPage>(`me/player/recently-played${params}`);
    }

    public async getUsersQueue() {
        return await this.getRequest<Queue>("me/player/queue");
    }
