...
describe(
    ...
    () => {
        ...
        beforeAll(async () => {
            ...
            wasArtistFollowed = (await sut.currentUser.followsArtistsOrUsers([artistId], "artist"))?.[0] ?? false;
            if (!wasArtistFollowed) {
                await sut.currentUser.followArtistsOrUsers([artistId], "artist");
            }

            albumId = validAlbumResult().id;
            wasAlbumSaved = (await sut.currentUser.albums.hasSavedAlbums([albumId]))?.[0] ?? false;
            if (!wasAlbumSaved) {
                await sut.currentUser.albums.saveAlbums([albumId]);
            }

            audioBookId = validAudioBook().id;
            wasAudioBookSaved = (await sut.currentUser.audiobooks.hasSavedAudiobooks([audioBookId]))?.[0] ?? false;
            if (!wasAudioBookSaved) {
                await sut.currentUser.audiobooks.saveAudiobooks([audioBookId]);
            }

            showId = validShow().id;
            wasShowSaved = (await sut.currentUser.shows.hasSavedShow([showId]))?.[0] ?? false;
            if (!wasShowSaved) {
        ...
        it("create and modify playlists for a user works", async () => {
            ...
            const playlist = await sut.playlists.getPlaylist(result.id);
            ...
            if (playlistWithoutTracks === null) {
                throw new Error("Playlist without tracks is null");
            }
            ...
            if (playlist2 === null) {
                throw new Error("Playlist 2 is null");
            }
